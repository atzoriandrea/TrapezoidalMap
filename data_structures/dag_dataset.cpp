...
//    std::vector<Trapezoid> traps;std::vector<std::list<Trapezoid>::iterator>& garbageCollector
...
void Dag::followSegment(cg3::Segment2d &segment, DagNodeArea &leaf){
    std::vector<DagNodeArea const *> leaves;
...
//        DagNode *& l = reinterpret_cast<DagNode*&>(lf->getT().getNode());
//        if(iter==0)
//            l = createLeftMost(segment, *lf, garbage);
//        else if(iter<leaves.size()-1)
//            l = createIntermediate(segment, *lf, garbage);
//        else
//            l = createRightMost(segment, *lf, garbage);
//    }
...
DagNode *Dag::createRightMost(cg3::Segment2d &segment, DagNodeArea &leaf, std::vector<std::list<Trapezoid>::iterator>& garbageCollector)
{
    ...
    if(upper){
        DagNode*& ex = reinterpret_cast<DagNode*&>(trap.getLeftUp().getLeftUp().getRightUp().getNodeRef());
        ex = si->getLeftChild();
    }
    else{
        DagNode*& ex = reinterpret_cast<DagNode*&>(trap.getLeftDown().getLeftDown().getRightDown().getNodeRef());
        ex = si->getRightChild();
    }
    return qi;
}
